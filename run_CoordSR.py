...
# 랜덤한 데이터 100개 생성
    ...
    sampling_functions = {
        "uv": sampling_val_uv,
        "us_temp": sampling_val_us,
        "us": sampling_val_us_SR
    }
    ...
    # Optimizer & Loss
    optimizer = torch.optim.Adam(denoiser.parameters(), lr=5e-4)
...
# Scheduler: CosineAnnealingLR
    ...
    for epoch in tqdm(range(num_epochs), desc="Epochs"):
        ...
        for idx, (coords, image) in enumerate(tqdm(pseudo_loader, desc=f"Epoch {epoch+1}/{num_epochs}", leave=False)):
            ...
            #breakpoint()
            # 1) 모델(INR_R2L)로부터 noisy rgb 얻기
            #    coords는 list of 2 tensors이므로, 그대로 model(coords)에 넣어줌
        
            with torch.no_grad():  # model은 Freeze(학습 X)
                rgb_noisy = model(coords)  
            B = image.size(0)

            #breakpoint()
            rgb_noisy = rgb_noisy.reshape((B, scaled_h, scaled_w, cnn_input_channels))  # (B,H,W,3)
            rgb_noisy = rgb_noisy.permute(0, 3, 1, 2)     # (B,3,H,W)
            
            # 2)Denoiser 통과
            #breakpoint()
            denoised = denoiser(rgb_noisy)  # (B,3,H,W)
        ...
        if epoch % opt.test_freq ==0 and epoch != 0:
                ...
                for idx in val_idx:
                    ...
                    rgb_noisy = model(coords)
                    rgb_noisy = rgb_noisy.reshape((1,scaled_h, scaled_w, cnn_input_channels)).permute(0,3,1,2)
                    
                    torch.cuda.synchronize()
                    start.record()
                    denoised = denoiser(rgb_noisy)
                    end.record()
                    torch.cuda.synchronize()
