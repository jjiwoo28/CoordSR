stanford_path="/data/NeuLF_rgb/stanford_half" #�����ͼ� ���


test_day="250317_MLP_and_R2L_body" #������ ���� (json���� ) �߰� �̸��� �����մϴ�.
result_path="/data/result/${test_day}" #���� ��� ���� ���� ��� , �⺻�����δ� �ֻ��� data ������ ���� ������ �����Ǿ��ֽ��ϴ�.
                                                ...
                                                for res_width in "${res_widths[@]}"; do
                                                    for after_network_type in "${after_network_types[@]}"; do
                                                        for cnn_type in "${cnn_types[@]}"; do
                                                            for after_network in "${after_networks[@]}"; do
                                                                ...
                                                                python run_R2L_body.py \
                                                                    --data_dir "${stanford_path}/${dataset}" \
                                                                    --pseudo_data_path "${teacher_header}${dataset}${teacher_footer}/pseudo_data" \
                                                                    --exp_dir "${result_path}/${test_day}_${nonlin}_d${depth}_w${width}_cd${coord_depth}_cd${coord_width}_R${R}_${batch_size}_decom_dim_${decom_dim}_lr${lr}_${dataset}_cnn_depth_${res_depth}_cnn_width_${res_width}_after_network_type_${after_network_type}_cnn_type_${cnn_type}_scale_${scale}_after_network_${after_network}" \
                                                                    --depth $depth \
                                                                    --width $coord_width \
