...
from model_all import LightFieldDataSet, HNeRV, HNeRVDecoder, TransformInput, LightFieldTeacherDataSet
...
def main():
    ...
    # lightfield
    parser.add_argument('--grid_size', default=17, type=int)
    ...
    parser.add_argument('--ch_in', type=int, default=2, help='input channel number')
...
def train(local_rank, args):
    ...
    # setup dataloader    
    lightfield_dataset = LightFieldDataSet(args)
    ...
    if args.use_lightfield_val and hasattr(args, 'grid_size') and args.grid_size > 0:
        args.val_ind_list = [72, 76, 80, 140, 144, 148, 208, 212, 216]
        
        print(f"Lightfield validation: {len(args.val_ind_list)} 프레임을 validation set으로 설정합니다.")
        val_dataset = Subset(lightfield_dataset, args.val_ind_list)
    ...
    if args.eval_only:
        ...
        for batch_ind, sample in enumerate(val_dataloader):
            ...
            if 'pe' in args.embed:
                ...
                cur_input = torch.cat([norm_y_unsqueezed, norm_x_unsqueezed], dim=1)
            else:
                cur_input = img_data
