...
from model_all import LightFieldDataSet, HNeRV, HNeRVDecoder, TransformInput, LightFieldTeacherDataSet, LightFieldVideoDataSet
...
def main():
    ...
    parser.add_argument('--val_path', type=str, default='', help='data path for vid')
    ...
    parser.add_argument('--ch_in', default=3, type=int)

    # 새로운 argument 추가
    parser.add_argument('--save_quant_model', action='store_true', default=False, help='양자화된 모델과 디코더를 저장합니다')
...
def train(local_rank, args):
    ...
    # setup dataloader    
    full_dataset = LightFieldVideoDataSet(args)
    # full_dataset = LightFieldDataSet(args)
    # full_dataset = LightFieldTeacherDataSet(args)
    ...
    if args.use_lightfield_val and hasattr(args, 'grid_size') and args.grid_size > 0:
        args.val_ind_list = []
        for yy in range(2, 6+1, 2):
            for xx in range(2, 6+1, 2):
                for tt in range(0, full_dataset.frame_num):
                    val_i = (yy * 9 + xx) * full_dataset.frame_num + tt
        ...
        train_ind_list = [i for i in range(args.full_data_length)]
    else:
    ...
    if args.eval_only:
        ...
        # gt와 model의 이미지 저장
        for batch_ind, sample in enumerate(val_dataloader):
            device = next(model.parameters()).device
            img_gt = sample['img']
            norm_y, norm_x = data_to_gpu(sample['norm_y'], device), data_to_gpu(sample['norm_x'], device)
            norm_t = data_to_gpu(sample['norm_idx'], device)
            print(norm_y, norm_x)
            if 'pe' in args.embed:
                norm_y_unsqueezed = norm_y.unsqueeze(1)
                norm_x_unsqueezed = norm_x.unsqueeze(1)
                norm_t_unsqueezed = norm_t.unsqueeze(1)
                
                cur_input = torch.cat([norm_y_unsqueezed, norm_x_unsqueezed, norm_t_unsqueezed], dim=1)
            else:
                cur_input = img_data
            img_out, _, _ = model(cur_input)  # 모델의 출력 이미지
            
            # 이미지 저장 경로 설정
            gt_filename = f'{args.outf}/gt_{batch_ind:04d}.png'
            out_filename = f'{args.outf}/out_{batch_ind:04d}.png'
            
            # 이미지 저장
            save_image(img_gt, gt_filename)
            save_image(img_out, out_filename)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or (args.epochs - epoch) in [1, 3, 5]:
            results_list, hw = evaluate(model, val_dataloader, local_rank, args, 
...
# 새로운 함수 추가
def save_quantized_model(model, quant_embed, quant_ckt, vid_embed, args, local_rank=None):
    """
    양자화된 모델과 일반 모델을 저장하는 함수
    """
    if local_rank in [0, None] and quant_ckt != None:
        ...
        print(f"양자화된 모델이 {args.outf}/quant_vid.pth에 저장되었습니다.")
        print(f"디코더가 {args.outf}/img_decoder.pth에 저장되었습니다.")
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        for i, sample in enumerate(full_dataloader):
            img_data, img_idx = data_to_gpu(sample['img'], device), data_to_gpu(sample['img_idx'], device)
            norm_y, norm_x = data_to_gpu(sample['norm_y'], device), data_to_gpu(sample['norm_x'], device)
            norm_t = data_to_gpu(sample['norm_idx'], device)
            ...
            # img_gt와 img_out를 디스크에 저장합니다
            if args.dump_images: 
                for batch_ind, cur_img_idx in enumerate(img_idx):
                    idx_value = int(cur_img_idx.item())
                    # 고유한 파일 이름 생성
                    gt_filename = f'{gt_image_dir}/gt_{idx_value:04d}.png'
                    out_filename = f'{out_image_dir}/out_{idx_value:04d}.png'
                    
                    # img_gt와 img_out 차원 형식: [배치, 채널, 높이, 너비]
                    save_image(img_gt[batch_ind], gt_filename)
                    save_image(img_out[batch_ind], out_filename)
                    
                    # 처음 이미지에 대한 정보 출력
                    if i == 0 and batch_ind == 0:
                        print(f"Ground truth 이미지 저장 경로: {gt_filename}")
                        print(f"Output 이미지 저장 경로: {out_filename}")
                        print(f"이미지 형식: {img_gt.shape} (배치, 채널, 높이, 너비)")
            ...
            if args.eval_fps:
                ...
                for _ in range(100):
                    ...
                    # if device.type.startswith('cuda'):
                        ...
                        # print(f"GPU 연산 시간: {elapsed_time} ")
                    time_list.append(dec_time)
            ...
            if i % args.print_freq == 0 or i == len(full_dataloader) - 1:
                ...
                print_str = '[{}] Rank:{}, Eval at Step [{}/{}] ,avg_time: {:.4f}, FPS {}, '.format(
                    datetime.now().strftime("%Y/%m/%d %H:%M:%S"), local_rank, i+1, len(full_dataloader), avg_time, round(fps, 1))
        ...
        # 원래 이 부분에 있던 모델 저장 코드를 함수 호출로 대체
        # dump quantized checkpoint, and decoder
        if args.save_quant_model:  # 새로운 인자를 추가하여 선택적으로 저장할 수 있게 함
            save_quantized_model(model, quant_embed, quant_ckt, vid_embed, args, local_rank)
        
        # 허프만 코딩은 그대로 유지
        # huffman coding
        if local_rank in [0, None] and quant_ckt != None and huffman_coding:
