...
class COORDX(nn.Module):
    def __init__(
        ...
        if self.after_network_type == "MLP":
            ...
            self.after_network = MLPWithSkips(
                ...
                skips=[ 8, 16, 24, 32]  # 원하는 대로 설정 (예: 2의 배수, 4의 배수 등)
            )
        elif self.after_network_type == "None":
            self.after_network = nn.Linear(coord_hidden_features, out_features)

        elif self.after_network_type == "rgb" or self.after_network_type == "feature":
            self.after_network = nn.Linear(coord_hidden_features, out_features)
        ...
        if self.isAddon:
            # coord_input_layer와 coord_net의 파라미터를 고정
            for i in range(len(self.coord_input_layer)):
                for param in self.coord_input_layer[i].parameters():
                    param.requires_grad = False
                    
            for layer in self.coord_net:
    ...
    def forward_fusion(self, hs):
        ...
        elif self.after_network_type == "rgb":
            h = self.after_network(h)
            h = h.squeeze(0)
            
        elif self.after_network_type == "feature":
            h = h.squeeze(0)
